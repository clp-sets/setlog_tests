...
:- object(tests,
	...
	test(setlog_01, true(Result == failure), [note(time-Time)]) :-
		benchmark_reified(
			setlog(
	...
	test(setlog_02, true(Result == failure), [note(time-Time)]) :-
		benchmark_reified(
			setlog(
	...
	test(setlog_03, true(Result == failure), [note(time-Time)]) :-
		benchmark_reified(
			setlog(
	...
	test(setlog_04, true(Result == failure), [note(time-Time)]) :-
		benchmark_reified(
			setlog(
